...
public class BaseDAO {
 ...
 protected final Connection getTargetConnectionByServiceName(String driverPrefix, String url, int port, String servicenameOrSID, String username, String password) {
 Connection conn = null;
 try{
	 conn = DriverManager.getConnection(driverPrefix+url+":"+port+"/" + servicenameOrSID, username, password); //if SID, use : instead of / (after port) ALSO: other databases may have it differently
 }
 catch(SQLException e){
	 throw new RuntimeException(e);
 }
